...
bd_canal %>% 
  ...
  ggplot() +
  annotate(geom = "rect", xmin = 1, xmax = 37, ymin = 0, ymax = 2500, fill = "salmon") +
  ...
  geom_vline(xintercept = c(1, seq(5, 30, 5), 37), color = "white", linetype = 3, size = 0.4) +
  ...
  ggsave("03_graficas/corredor_endemico_casos_confirmados_dengue_semana_37.png", width = 13.2, height = 10, dpi = 200)
...
bd_canal %>% 
  ...
  mutate(casos_esperados = exp(media_geom) - 1,
         ...
         inferior = exp(limite_inf) - 1) %>% 
  ggplot() +
  annotate(geom = "rect", xmin = 1, xmax = 37, ymin = 0, ymax = 20000, fill = "salmon") +
  geom_area(aes(semana, superior), fill = "#fed976") +
  geom_area(aes(semana, casos_esperados), fill = "#74c476") +
  geom_area(aes(semana, inferior), fill = "#6baed6") +
  geom_vline(xintercept = c(1, seq(5, 30, 5), 37), color = "white", linetype = 3, size = 0.4) +
  ...
  theme(panel.background = element_rect(fill = "transparent", color = "transparent"),
        panel.grid = element_line(color = "grey20", linetype = 3, size = 0.4),
        axis.ticks = element_line(color = "grey20", linetype = 3, size = 0.4)) +
  ggsave("03_graficas/corredor_endemico_acumulado_casos_confirmados_dengue_semana_37.png", width = 13.2, height = 10, dpi = 200)
